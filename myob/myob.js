...
const handleLine = function(inputLine){
	//TODO: data clean
	if(!inputLine || inputLine.length < 10){
		return "error in line" + "-" + inputLine;
	}
...
// Output (name, pay period, gross income, income tax, net income, xz):
	let retArr = [];
	retArr.push(obj.name);
	retArr.push(obj.payPeriod);
	retArr.push(obj.grossIncome);
	retArr.push(obj.incomeTax);
	retArr.push(obj.netIncome);
	retArr.push(obj.super);
	return retArr.join(",");
}
